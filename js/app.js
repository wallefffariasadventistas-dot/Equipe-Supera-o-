...
function renderPreenchimentoDiario() {
  ...
  if (!nao.length) { listaNao.innerHTML='<div style="text-align:center;padding:16px;color:var(--verde-claro);font-size:13px;font-weight:700;">🎉 Todos preencheram!</div>'; }
    ...
    nao.forEach(u=>{
      ...
      const div=document.createElement('div');
      div.style.cssText='display:flex;align-items:center;gap:10px;padding:10px 12px;background:rgba(255,109,0,0.07);border:1px solid rgba(255,109,0,0.2);border-radius:10px;';
      div.innerHTML=`<div style="width:36px;height:36px;border-radius:50%;background:rgba(255,109,0,0.15);border:1px solid rgba(255,109,0,0.3);display:flex;align-items:center;justify-content:center;font-weight:700;font-size:13px;color:var(--laranja);">${escapeHtml(iniciais(u.nome))}</div><div style="flex:1;"><div style="font-size:14px;font-weight:700;color:var(--branco);">${escapeHtml(u.nome)}</div><div style="font-size:12px;color:var(--texto3);margin-top:2px;">📞 ${escapeHtml(u.tel)||'—'}</div></div>${waLink?`<a href="${waLink}" target="_blank" style="padding:6px 10px;background:rgba(37,211,102,0.15);border:1px solid rgba(37,211,102,0.3);border-radius:8px;color:#25D366;font-size:11px;font-weight:700;text-decoration:none;white-space:nowrap;">📲 Notificar</a>`:''}`;
